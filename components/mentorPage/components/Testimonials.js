        ...
        <div className='tw-group tw-max-w-[1300px] tw-flex tw-flex-col tw-w-full tw-h-full tw-relative tw-px-4 tw-items-center tw-justify-center'>
            ...
            <div>
                <div className='tw-flex tw-flex-col tw-items-center tw-justify-center tw-text-center tw-mt-5'>
                    <h1 className='tw-text-2xl tw-font-bold max-[637px]:tw-text-xl tw-text-gray-800'>{data[currentIndex].testimonialUserName}</h1>
                    <h2 className='tw-text-lg max-[637px]:tw-text-sm tw-text-gray-500'>{data[currentIndex].testimonialUserHeadline}</h2>
                    ...
                    <p className='tw-text-gray-600 tw-mt-5 tw-max-w-[990px] max-[637px]:tw-text-xs'>{data[currentIndex].testimonialDescription}</p>
                </div>
            </div>
                ...
                {data.map((testimonial, index) => {
                    return (
                        <RxDotFilled
                            key={index}
                            size={30}
                            className={`tw-cursor-pointer tw-mx-2 ${index === currentIndex ? 'tw-text-gray-600' : 'tw-text-gray-400'}`}
