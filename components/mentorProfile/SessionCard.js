...
export default function SessionCard( { type, name, description, duration, pricePerSession, handleBookSession } ) {
  return (
    <>
      {/* session card */}
      <div className="tw-flex tw-flex-row tw-w-full tw-p-8 tw-bg-white tw-rounded-[36px] tw-shadow-[0px_0px_40px_0px_#00000025] tw-gap-5 tw-border-t-[1px] tw-mb-[30px]">
        {/* Session card Icon */}
        { type === "video-meeting" ? <MdVideoCameraFront className="tw-text-[36px] tw-text-[#4338CA]"/>: <MdRingVolume className="tw-text-[36px] tw-text-[#4338CA]"/>}
        <div className="tw-flex tw-flex-col tw-w-full">
          {/* Session card title */}
          <h1 className="tw-text-[36px] tw-font-semibold tw-text-[#4338CA]">{name}</h1>
          {/* Session card description */}
          <h3 className="tw-text-[16px] tw-font-semibold tw-text-[#4338CA]/80">{description}</h3>
          {/* Session card duration */}
          <p className="tw-flex tw-flex-row tw-items-center tw-gap-3 tw-text-[16px] tw-whitespace-nowrap tw-font-medium tw-text-black/50"><BsClock /> {duration} mins</p>
          {/* Session card Charge */}
          <p className="tw-flex tw-flex-row tw-items-center tw-gap-3 tw-text-[16px] tw-whitespace-nowrap tw-font-medium tw-text-black/50"><BsCurrencyRupee /> {pricePerSession} mins</p>
          {/* Session Book Button */}
          <button
            type="button" 
            role="button" 
